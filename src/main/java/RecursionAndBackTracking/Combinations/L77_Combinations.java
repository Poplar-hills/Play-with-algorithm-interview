...
public class L77_Combinations {
     ...
     *                                   从[1,2,3,4]中取2个数
     *               1/                2/                  3\                4\
     *      从[2,3,4]中取1个数    从[1,3,4]中取1个数     从[1,2,4]中取1个数   从[1,2,3]中取1个数
     *     2/      3|     4\        3/    4\               4|
     *   解[1,2] 解[1,3] 解[1,4]  解[2,3] 解[2,4]          解[3,4]
     *
    ...
    public static List<List<Integer>> combine2(int n, int k) {
        List<List<Integer>> res = new ArrayList<>();
        if (n <= 0 || k <= 0 || k > n) return res;
        helper2(n, k, 1, new ArrayList<>(), res);
        return res;
    }

    private static void helper2(int n, int k, int i, List<Integer> list, List<List<Integer>> res) {
        if (list.size() == k) {
            res.add(new ArrayList<>(list));
            return;
        }
        for (int j = i; j <= n - (k - list.size()) + 1; j++) {  // 通过限制 j 的上限进行剪枝
            list.add(j);
            helper2(n, k, j + 1, list, res);
            list.remove(list.size() - 1);
        }
    }

    public static void main(String[] args) {
        log(combine2(4, 2));  // expects [[2,4],[3,4],[2,3],[1,2],[1,3],[1,4]]
        log(combine2(4, 1));  // expects [[1],[2],[3],[4]]
    }
}
