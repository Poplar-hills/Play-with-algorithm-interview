 ...
 *     选用相同的元素而产生相同分支（见解法4的图）∴ 在 L46_Permutations 的基础上，对每层递归内分支时选用的元素进行去重。在实现时，
 *     要么在最后用 Set 对解进行去重，要么在递归内部进行分支时通过避免选用重复元素达到剪枝的目的。
 * */
...
public class L47_PermutationsII {
    ...
    /*
     * 解法4：Recursion + Backtracking + Inner Set (🥇最优解)
     * - 思路：与解法1一致。
     ...
     *   则得到的分支一定重复 ∴ 可以在解法1的基础上在每层递归内使用 Set 对 nums[i] 进行判断。剪枝后的树变成这样：
     *                              []
     *                  1/          2|          1\
     *              [1]             [2]          x
     *           2/     1\       1/    1\
