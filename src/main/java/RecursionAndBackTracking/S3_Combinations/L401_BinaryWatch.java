 ...
 *   represent the minutes (0-59). Each LED represents a zero or one, with the least significant bit on the
 *   right. For example: the following binary watch reads "3:25".
 *                +------------+--------------------+
...
public class L401_BinaryWatch {
    ...
    private static List<Integer> selectToSum(int[] nums, int n, int limit) {  // 从 nums 中选出 n 个数来求和，且和要 < limit
        List<Integer> res = new ArrayList<>();                              // 复杂度为 O(C(len(nums), n))
        backtrack(nums, n, limit, 0, 0, res);
        return res;
    }

    private static void backtrack(int[] nums, int n, int limit, int i, int sum, List<Integer> res) {
        if (n == 0) {
        ...
        for (int j = i; j < nums.length; j++)  // ∵ 要不重复地选取 ∴ j 要从 i 开始，并且进入下层递归时要 j+1
            if (sum + nums[j] < limit)
                backtrack(nums, n - 1, limit, j + 1, sum + nums[j], res);
    }
    ...
    public static void main(String[] args) {
        ...
        log(readBinaryWatch(10));
        // expects []

        log(readBinaryWatch(8));
        // expects ["11:59", "11:55", "11:47", "11:31", "7:59", "7:55", "7:47", "7:31"]
    }
}
