...
public class L39_CombinationSum {
     ...
     *
     * - 💎 总结：
     *   1. 为了不产生重复解，需让每个节点在遍历 nums 时不回头，只遍历索引 >= 当前节点的 nums ∴ 在每层递归内的 for 上让 j ∈ [i,n)；
     *   2. 又 ∵ 题中说了 nums 中的元素可以使用无限多次 ∴ 纵向上同一分支上的各层递归都要相同的索引开始遍历，如 👆在给最左边的6节点、
     *      4节点、2节点分支时，都要包含上层已经使用过的2分支；而在给最右侧的5节点分支时也要包含上层用过的3分支。包含 ∴ 纵向各层递归
     *      之间要传的是 j，而非 j+1（这是与 L40 的不同点）。
     ...
     * - 思路：与解法1、2一致。
     * - 实现：解法2通过不回头的遍历 nums 进行了进一步剪枝，而该解法更进一步，再添加一种剪枝的情况，进一步提升效率。例如
