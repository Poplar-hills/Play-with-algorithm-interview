...
public class L78_Subsets {
    ...
    /*
     * 解法2：Iteration
     * - 思路：对于 nums 中的每个元素都有用或不用两种选择，例如对于 [1,2,3] 来说，最初有空集解 []，之后：
     *   - 若用1，则将1追加到 [] 中，得到解 [1]；若不用1，则还是 []；
     *     合并两部分得到 [[],[1]]。
     *   - 若用2，则将2追加到 [[],[1]] 中的每个解里，得到解 [[2],[1,2]]；若不用2，则还是 [[],[1]]；
     *     合并两部分得到 [[],[1],[2],[1,2]]。
     *   - 若用3，则将3追加到 [[],[1],[2],[1,2]] 中的每个解里，得到解 [[3],[1,3],[2,3],[1,2,3]]；若不用3，则还是
