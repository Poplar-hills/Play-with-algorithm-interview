...
public class L40_CombinationSumII {
     ...
     * - 思路：L39_CombinationSum 中的限制是不能产生重复解，而该题中除了该限制之外还多了不能重复使用 candidate 的限制。因此：
     *     - 为了不重复使用 candidate，需在向下递归的过程中，逐层右移 candidates 上的指针 i；
     *     - 为了不产生重复解，还需对 candidates 排序，并在每次遍历时判断是否遍历到了相同的元素。
    ...
    private static void helper(int[] candidates, int target, int i, List<Integer> list, List<List<Integer>> res) {
        if (target == 0) {
        ...
        for (int j = i; j < candidates.length && target >= candidates[j]; j++) {
            ...
            list.add(c);
            helper(candidates, target - c, j + 1, list, res);  // 下一层递归从第 j+1 个元素开始遍历，以保证不使用已用过的元素
