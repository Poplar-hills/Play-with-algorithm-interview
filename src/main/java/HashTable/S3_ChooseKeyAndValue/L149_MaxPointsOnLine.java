...
public class L149_MaxPointsOnLine {
    /*
     * 解法1：
     * - 思路：该问题的关键在于如何判断多个点在一条直线上。首先想到可以通过两点一线的斜率（slope）判断 —— 遍历所有点的两两组合，
     *   用 Map 记录 <斜率, 点个数>。但根据公式 y = kx + b，若两条线斜率 k 相等，但偏移量 b 不等，则这4个点并不在一条直线
     *   上，该方法没有考虑这种情况 ∴ 更换思路 —— 为每个点单独创建一个 Map，生成该点与所有其他点连成的直线的 <斜率, 点个数>。
     *   而所有这些 Map 中的最大 value 即是所求解。
