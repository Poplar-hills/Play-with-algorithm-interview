...
public class L16_3SumClosest {
     ...
     *   求解 —— 通过在遍历中挨个固定元素，将 3Sum 化简为 2Sum 问题。而一但问题变成了 2Sum，就可以考虑 2Sum 的三种经典思路：
     *   1. 指针对撞；2. 二分查找；3. 查找表。由于本题条件是最接近 target，而非等于 target ∴ 无法使用二分查找、查找表这类精确
     *   搜索的方式，只能使用指针对撞来对逼近 target，并沿途判断哪个是最接近的解。
    ...
    public static int threeSumClosest2(int[] nums, int target) {
        ...
        Arrays.sort(nums);
        int closest = nums[0] + nums[1] + nums[2];  // 也可以初始化为 Integer.MAX_VALUE
        int n = nums.length;
