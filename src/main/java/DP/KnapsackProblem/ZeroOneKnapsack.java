...
*     2. 定义子问题，即确定状态转移方程中的函数定义 —— f(i, j) 表示“当背包剩余容量为 j 时，从前 i 个物品中能得到的最大价值”。
*     3. 推导状态转移方程：
*        - ∵ 对每种物品都有放/不放两种选择 ∴ f 所求的“最大价值”一定来自于其中价值更大的那个选择；
*          - 若不放物品 i，则最大价值为 f(i-1, j)，即从前 i-1 个物品中能得到的最大价值；
*          - 若放入物品 i，则最大价值为 v(i) + f(i-1, j-w(i))，即当前物品价值 + 往剩余容量中尝试放入前 i-1 个物品能得到的最大价值；
...
public class ZeroOneKnapsack {
    ...
    * */
    ...
    public static void main(String[] args) {
        log(knapsack4(new int[]{1, 2, 3}, new int[]{6, 10, 12}, 5));       // expects 22.
        log(knapsack4(new int[]{1, 3, 4, 2}, new int[]{3, 9, 12, 8}, 5));  // expects 17.
    }
}
