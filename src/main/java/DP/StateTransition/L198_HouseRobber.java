...
public class L198_HouseRobber {
    ...
    *                  max[2..n)        max[3..n)            max[]
    *             偷2号/   偷3号|     \           偷3号/         \
    *     [4..n)内的最大收获   ...    ...  [5..n)内的最大收获   ...
    *
    *   说明：要求 [0..n) 内的最大收获，第一次偷共有 n 种方案：偷0号、偷1号、……、偷n-1号。而这 n 种方案分别对应了 n 个子问题：
    *
    *
    *
    *
    *这样的分解的含义：[0..n)内的最多收获 = Max((偷0号收获 + [2..n)内的最多收获), (偷1号收获 + [3..n)内的最多收获), ..., 偷n-1号收获)
