 ...
 *       - 定义子问题：f(i, j) 表示“用前 i 个物品填充剩余容量为 j 的背包所能得到的最大价值”。
 *       - 写出状态转移方程：
 *         - ∵ 对任意一件物品 i 都有放/不放两种选择 ∴ f(i, j) = max(放 i 能得到的最大价值, 不放 i 能得到的最大价值)。
 *         - 放入 i 能得到的最大价值为 v[i] + f(i-1, j-w[i]))，即物品 i 的价值 + 用前 i-1 个物品中填充剩余容量所能得到的最大价值；
 ...
 *      其中，0) 是 1) 的子问题，通过求 0) 的最优解可获得 1) 的最优解；1) 是 2) 的子问题，通过求 1) 的最优解可获得 2) 的最优解……
 *      可见，在该递推过程中“局部问题”的范围逐渐增大，直到覆盖全局“全局问题”为止。若是采用贪心，则会是：
 *        0). 用性价比最高的物品填充剩余容量；
 *        1). 用性价比第2高的物品填充剩余容量；
 *        3). ...
 *      该过程中不存在递推关系，后一个问题的解不建立在前一个问题的解之上，每个问题都是关注的是个体，因此每个“局部问题”的范围都是一样的。
 * */
...
public class B1_ZeroOneKnapsack {
     ...
     * 解法2：DP
     * - 思路：
     *   1. ∵ 该问题中有两个变量 ∴ 状态转移过程可以通过填表法（tabulation）来可视化。动画演示 SEE:
    ...
    public static int knapsack2(int[] w, int[] v, int c) {
        ...
        for (int i = 1; i < n; i++) {
            for (int j = 1; j <= c; j++) {
                if (j < w[i])           // 若放不下当前物品，则能得到的最大价值就是上一个容量的最大价值
                    dp[i][j] = dp[i - 1][j];
                else                    // 若放得下当前物品，则需重新比较
