...
public class L123_BestTimeToBuyAndSellStockIII {
     ...
     *    0 + - - - - - - * - - - - - - - - - - - - - - - -
     *        0     1     2     3     4     5     6     7
     *   在 L122 中 ∵ 可以交易任意多次 ∴ 只要将所有单次上涨的幅度相加即是最大利润。而该题中 ∵ 要在最多交易2次的情况下找到
     *   最大利润 ∴ 就需要考虑跨越 valley 的上涨的情况 —— 是2个单次上涨幅度相加更大，还是一个跨越 valley 的上涨幅度（如
     *   [2,5]）再加另一个上涨幅度更大 —— 这就无法像 L122 解法1中那样仅使用遍历就能求解，而是需要使用 DP 才行。
     *
     *   ∵ 存在股价和交易次数2个变量 ∴ 在使用 DP 时需要同时考虑这两个变量 ∴ 需要使用二维 dp 数组：maxProfit[t][d] 表示
     *   “在第 d 天，最多交易 t 次时所能获得的最大的利润”。由此列出二维表：
     *       t\p [ 3,  5,  0,  3,  2,  5,  2,  5 ]
     ...
     *     - 若在第 d 天不卖，最多交易 t 次时所能获得的最大利润
     *           = 前一天最多交易 t 次所能获得的最大利润
     *           = maxProfit[t][d-1]
     *     - 若在第 d 天卖出，最多交易 t 次时所能获得的最大利润
     *           = 第 d 天的股价 + 在前 d-1 天内买入且之前最多交易 t-1 次所能获得的最大利润
     *           = 第 d 天的股价 + 比较并选出 [0,d-1] 区间内每天买入且之前最多交易 t-1 次所获最大利润中的最大的那个
    ...
    public static int maxProfit(int[] prices) {
        ...
        for (int t = 1; t < dp.length; t++) {           // 固定交易次数（t=0 时最大利润都是0 ∴ 可跳过）
            ...
            for (int d = 1; d < n; d++) {
                maxProfitAfterBuy = Math.max(maxProfitAfterBuy, dp[t - 1][d - 1] - prices[d - 1]);
                dp[t][d] = Math.max(dp[t][d - 1], maxProfitAfterBuy + prices[d]);
            }
        }
