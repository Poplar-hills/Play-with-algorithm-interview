...
public class L560_SubarraySumEqualsK {
     ...
     *   > k 时收缩来判断。但在 test case 4-5 中，由于存在负数，使该判断方式失灵 ∴ 该题无法使用滑动窗口求解。
     * */
     ...
     *   （即 nums[l..r] 之和）。而要快速得到任意 nums[l..r] 之和，则要使用 Prefix sums，用 preSums[i] 来表示 nums[0..i] 之和。
    ...
    public static int subarraySum2(int[] nums, int k) {
        ...
        for (int l = 0; l < n; l++)     // 双重循环遍历所有 subarray 检查是否有符合条件的 subarray
            for (int r = l; r < n; r++)
                if (preSums[r] - preSums[l] + nums[l] == k)  // 用 preSums 获取 nums[l,r] 内的元素和
    ...
    /*
     * 解法3：累加 + Two Sum 查找表
     * - 💎 思路：解法2通过双重循遍历所有的 subarray 来查找是否存在 preSums[0,r] - preSums[0,l-1] == k，该过程其实是一个
     *   典型的 Two Sum 查找问题 ∴ 可以采用 L1_TwoSum 解法4的查找表思路将时间复杂度再降低一个次方。
     * - 实现：在单次遍历中：
     *    1. 一边累加 preSums[0,r]（即 sum）；
     *    2. 一边检查 preSums[0,r] 的 complement（即 preSums[0,l-1] = preSums[0,r] - k = sum - k）是否存在于查找表中
     *       （注意不能是 k - sum），若存在则将 complement 在查找表中的值累加到结果上；
     ...
     *   （即让 preSums[0,r] - preSums[0,l-1] = k 成立的 preSums[0,l-1] 个数），例如👆最后一行中，sum=10，complement=5，
     *   此时 complement 在 Map 中的值为2的含义就是"让 preSums[0,5] - preSums[0,l-1] = 5 成立的 preSums[0,l-1] 个数为2"，
     *   因此要把这个个数加到结果 count 上（而不让 count++）。
