...
public class L560_SubarraySumEqualsK {
    ...
    public static int subarraySum2(int[] nums, int k) {
        ...
        for (int l = 0; l < n; l++)     // 双重循环遍历所有 subarray 检查是否有符合条件的 subarray
            for (int r = l; r < n; r++)
                if (preSums[r] - preSums[l] + nums[l] == k)  // 用 preSums 获取 [l,r] 内的元素和
     ...
     *    3. 若存在则将 complement 在 Map 中的值加到 count 上；
     *    3. 不管是否存在都更新 sum 在 Map 中的值。
     *   对于：[4,  2, -1,  5, -5,  5],  k = 5
     ...
     *
     * - 注意：代码中 count += 的必须是 sum-k 的频率，而不能是 count++。举例说明：在👆最后一行中，complement=5 在 Map
     *   中值为2的意义是“能让当前 sum 减掉它后等于 k（即 sum[0,i] - sum[0,j-1] == 5）的 subarray sum 共有2个”（sum[0,2]
