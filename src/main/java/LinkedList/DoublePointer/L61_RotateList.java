...
public class L61_RotateList {
    /*
    * 解法1：双指针
    * - 思路：要得到 rotate 之后的链表，关键在于要确定从哪里截断并链到头节点上去。例如 0 -> 1 -> 2 -> 3 -> 4，k=7，在将其
    *   rotate 之后得到 3 -> 4 -> 0 -> 1 -> 2，即从原链表2节点之后截断，并把截断的部分链回0节点上去。要确定从哪里截断则需要：
    *   1. 求去掉绕圈后的剩余距离，即 k % len = 2，说明链表中的各节点都需移动2步才能到达 rotate 之后他们各自的位置，也说明原
    *      链表中最后2个节点会被截断并链回头结点。至此，问题转化成了类似 L19 中的问题 ——
