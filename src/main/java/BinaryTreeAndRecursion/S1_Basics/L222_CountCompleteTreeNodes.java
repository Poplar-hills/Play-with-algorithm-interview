 ...
 *   2. 完全二叉树非最后一层的节点数为 2^(h-1)，最后一层的节点数为 1~2^h，其中 h 为层的深度；
 *   3. 完美二叉树的总结点数为 2^h - 1。
 * */
...
public class L222_CountCompleteTreeNodes {
     ...
     * - 思路：注意题中条件，这是一棵"complete binary tree" ∴ 不同于解法1、2、3，本解法利用了完全二叉树的2个性质：
     *     1. 完美二叉树的节点总数：2^h - 1，其中 h 为树高（每行的节点数为 2^(h-1)）；
